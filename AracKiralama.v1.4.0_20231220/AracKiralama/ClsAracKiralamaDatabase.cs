...
namespace AracKiralama
{
    public class ClsAracKiralamaDatabase
    {
        ...
        public void UpdateCar(string Model, string Renter, string Description, string RentDate, string DeliveryDate)
        {
            ...
            {
                SqlDatabase.ConOpen();
                SqlDatabase.cmd = new SqlCommand("USE AracKiralama UPDATE Cars SET Rented = 1, Renter = @Renter, Description = @Description, RentDate = @RentDate, DeliveryDate = @DeliveryDate WHERE Model = @Model", SqlDatabase.con);
                SqlDatabase.cmd.Parameters.AddWithValue("@Renter", Renter);
                SqlDatabase.cmd.Parameters.AddWithValue("@Description", Description);
                SqlDatabase.cmd.Parameters.AddWithValue("@RentDate", RentDate);
                SqlDatabase.cmd.Parameters.AddWithValue("@DeliveryDate", DeliveryDate);
        ...
        // Recall Car
        public void RecallCar(string Damaged, string Description, string DeliveredDate, string Model, string Renter)
        {
            SqlDatabase.ConOpen();
            SqlDatabase.cmd = new SqlCommand("USE AracKiralama UPDATE Cars SET Damaged = @Damaged, Description = @Description, Rented = 0, Renter = NULL, RentDate = NULL, Delivered = 1, DeliveredDate = @DeliveredDate, DeliveryDate = NULL WHERE Model = @Model AND Renter = @Renter", SqlDatabase.con);
            SqlDatabase.cmd.Parameters.AddWithValue("@Damaged", Damaged);
            SqlDatabase.cmd.Parameters.AddWithValue("@Description", Description);
            SqlDatabase.cmd.Parameters.AddWithValue("@DeliveredDate", DeliveredDate);
            SqlDatabase.cmd.Parameters.AddWithValue("@Model", Model);
            ...
            SqlDatabase.cmd = new SqlCommand("USE AracKiralama UPDATE Customers SET Renter = 0, RentedModel = NULL, RentDate = NULL, Delivered = 1, DeliveredDate = @DeliveredDate, DeliveryDate = NULL", SqlDatabase.con);
            SqlDatabase.cmd.Parameters.AddWithValue("@DeliveredDate", DeliveredDate);
        }
        #endregion
    }
