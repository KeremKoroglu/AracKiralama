        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            if (TbcEditCar.SelectedIndex == 0)
            {
                ...
                else
                {
                    ...
                    sqlDatabase.WriteCarTable(TxtPrice.Text, TxtModel.Text, TxtKm.Text, CmbSeat.Text, ChbAutoGear.Checked.ToString(), ChbDamaged.Checked.ToString(), RtbDescription.Text, ChbRented.Checked.ToString(), TxtRenter.Text, rentDate, ChbDelivered.Checked.ToString(), deliveredDate, deliveryDate);
                }
            }
            else if (TbcEditCar.SelectedIndex == 1)
            {         
                DatabaseManage.DeleteColumn(sqlDatabase.tbCars, (CmbCondition.Text, TxtValue.Text)); 
            }
