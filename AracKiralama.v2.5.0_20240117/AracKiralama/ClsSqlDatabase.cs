    ...
    public class ClsSqlDatabase
    {
            ...
            switch (ConditionCount)
                {
                    ...
                    case 1:
                        ...
                        Command = "USE " + Db + " SELECT * FROM " + Tb + " WHERE @Condition = @Value";
                        da.SelectCommand = new SqlCommand(Command, con);
        ...
        public int ColumnCount(string Db, string Tb, string Type = "=", byte ConditionCount = 0, string Condition = "", string Value = "", string Condition2 = "", string Value2 = "")
        {
            try
            {
                ...
                if (ConditionCount == 0)
                {
                    Command = "USE " + Db + " SELECT COUNT (*) AS Count FROM " + Tb;
                    cmd = new SqlCommand(Command, con);
                    dr = cmd.ExecuteReader();
                ...
                switch (Type)
                {
                    case "=":
                        ...
                        {                         
                            case 1:
                                Command = "USE " + Db + " SELECT COUNT (*) AS Count FROM " + Tb + " WHERE @Condition = @Value";
                                cmd = new SqlCommand(Command, con);
                            ...
                            case 2:
                                Command = "USE " + Db + " SELECT COUNT (*) AS Count FROM " + Tb + " WHERE @Condition = @Value AND @Condition2 = @Value2";
                                cmd = new SqlCommand(Command, con);
                                cmd.Parameters.AddWithValue("@Condition", Condition);
                                cmd.Parameters.AddWithValue("@Value", Value == null ? DBNull.Value : (object)Value);
                    ...
                    case "/%":
                        ...
                        {
                            case 1:
                                Command = "USE " + Db + " SELECT COUNT (*) AS Count FROM " + Tb + " WHERE @Condition LIKE '%@Value'";
                                cmd = new SqlCommand(Command, con);
                                cmd.Parameters.AddWithValue("@Condition", Condition);
